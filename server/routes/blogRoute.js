...
router.post('/add', authMiddleware, async (req, res) => {
  try {
    ...
    const blog_id = `${slugify(title, { lower: true, strict: true })}-${nanoid(6)}`;
...
router.get("/get-blog", async (req, res) => {
  ...
  try {
    const blog = await Blog.findOne({ blog_id: id })
      .populate('author', 'email firstName lastName image description tagline socialLinks') // Add author information
      .lean();;
